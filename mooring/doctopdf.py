...
def create_annual_admission_letter_old(booking):
    ...
    key = "{{customername}}"
    docx_replace_regex(doc, re.compile(r''+key) , '{} {}'.format(booking.details.get('first_name', ''), booking.details.get('last_name', '')), key,False)
    key = "{{customeraddress}}"
    if len(booking.details.get('postal_address_line_2', '')) > 0:
       docx_replace_regex(doc, re.compile(r''+key) , '{}\n{}'.format(booking.details.get('postal_address_line_1', ''), booking.details.get('postal_address_line_2', '')), key,False)
    else:
       docx_replace_regex(doc, re.compile(r''+key) , '{}'.format(booking.details.get('postal_address_line_1', '')), key,False)
    #key = "{{customersuburb}"
